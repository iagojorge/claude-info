...
app.post('/api/payment/pix', async (req, res) => {
  ...
  try {
    const body = {
      transaction_amount: parseFloat(process.env.PRODUCT_PRICE || '97.00'),
      description: process.env.PRODUCT_NAME || 'Motoelétrica Pro - Curso',
...
async function notifyCustomer({ name, email, paymentId }) {
  ...
  const body = {
    ...
    template_params: {
      to_name: name,
      to_email: email,
      order_amount: `R$ ${parseFloat(process.env.PRODUCT_PRICE || '97.00').toFixed(2)}`,
